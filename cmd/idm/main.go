...
func t9ManyDownloadsAndPauseResumes() {
	...
	e1 := internal.NewAddQueueEvent(
		...
		1,
		0,
		"C:/Users/Asus/Documents/GitHub/project-1/files",
		types.TimeInterval{
			Start: now.Add(-10 * time.Minute),
			End:   now.Add(10 * time.Minute),
		},
...
func t10TestingQueueDestination() {
...
func t11ChangeMaxInProgress() {
	...
	q.MaxBandwidth = 9 * 1024 * 1024
	internal.AddQueue(q)
	d1.Filename = "downloade1.bin"
	d1.Url = "http://127.0.0.1:8080"
	internal.AddDownload(d1, q.Id)
	d2.Filename = "downloade2.bin"
	d2.Url = "http://127.0.0.1:8080"
	internal.AddDownload(d2, q.Id)
...
func t12TestDeleteDownload() {
	...
	//eventsMap[9] = []internal.IDMEvent{internal.NewPauseDownloadEvent(0)}
	//eventsMap[18] = []internal.IDMEvent{internal.NewResumeDownloadEvent(0)}
	eventsMap[10] = []internal.IDMEvent{internal.NewDeleteDownloadEvent(0)}
	slog.Info(fmt.Sprintf("GOOZ %+v", eventsMap))
...
func main() {
	...
	// t10TestingQueueDestination()
	// t11ChangeMaxInProgress()
	t12TestDeleteDownload()
}
